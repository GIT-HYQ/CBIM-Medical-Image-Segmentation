...
from training.validation_clDice import validation, SoftclDiceMetric
...
# --- [修改：根据开关初始化 TopoLoss] ---
    criterion_topo = TopoLoss().cuda() if args.use_topo else None
    # ------------------------------------
    ...
    best_clDice = np.zeros(args.classes)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch+1) % args.val_freq == 0:
            ...
            dice_list_test, ASD_list_test, HD_list_test, IoU_list_test, ACC_list_test, SPE_list_test, SEN_list_test, cldice_list_test = validation(net_for_eval, valLoader, args)
            ...
            log_evaluation_result(writer, dice_list_test, ASD_list_test, HD_list_test, IoU_list_test, ACC_list_test, SPE_list_test, SEN_list_test, cldice_list_test, 'test', epoch, args)
            ...
            logging.info(f"Best SPE:{best_SPE.mean():.4f}, Best SEN:{best_SEN.mean():.4f}, Best clDice: {best_clDice.mean():.4f}")
    ...
    if os.path.exists(best_pth):
        ...
        best_Dice, best_ASD, best_HD, best_IoU, best_ACC, best_SPE, best_SEN, best_clDice = validation(net_for_eval, testLoader, args, mode="Testing")
    ...
    return best_Dice, best_HD, best_ASD, best_IoU, best_ACC, best_SPE, best_SEN, best_clDice
...
def train_epoch(trainLoader, net, ema_net, optimizer, epoch, writer, criterion, criterion_dl, criterion_topo, scaler, args):
    ...
    # --- [新增：clDice 记录器] ---
    cldice_meter = AverageMeter("clDice", ":.4f")
    # 初始化计算类（iter_3 兼顾速度与精度）
    cldice_evaluator = SoftclDiceMetric(iter_=3).cuda()

    progress = ProgressMeter(
        len(trainLoader) if args.dimension=='2d' else args.iter_per_epoch, 
        [batch_time, epoch_loss, topo_loss_meter, cldice_meter], 
    ...
    for i, inputs in enumerate(trainLoader):
        ...
        def compute_total_loss(result, label, topo_weight=0.1):
            ...
            if isinstance(result, (list, tuple)):
                ...
                # 2. 计算 Aux Loss (如果存在)
                if args.aux_loss: 
                    loss_aux = args.aux_weight[1] * (criterion(result[1], label.squeeze(1)) + criterion_dl(result[1], label))
                    loss_seg = args.aux_weight[0] * loss_seg + loss_aux
        ...
        with torch.no_grad():
            # 提取分割头的预测结果
            seg_out = result[0] if isinstance(result, (list, tuple)) else result
            # 将 Logits 转为概率
            if args.classes == 2:
                # 二分类取 sigmoid 后第一个通道
                seg_prob = torch.sigmoid(seg_out[:, 1:2, ...])
            else:
                # 多分类取 softmax 后血管通道
                seg_prob = F.softmax(seg_out, dim=1)[:, 1:2, ...]
            
            # 计算当前 Batch 的 clDice 分数
            cldice_score = cldice_evaluator(seg_prob, label.float())
            cldice_meter.update(cldice_score.item(), img.shape[0])
    ...
    # --- [新增：记录 clDice 平均值] ---
    writer.add_scalar('Train/clDice', cldice_meter.avg, epoch+1)
...
def get_parser():
    ...
    parser.add_argument('--use_topo', action='store_true', help='If true, use TopoLoss and Topological Mediator')
...
if __name__ == '__main__':
    ...
    Dice_list, HD_list, ASD_list, IoU_list, ACC_list, SPE_list, SEN_list, clDice_list = [], [], [], [], [], [], [], []
    ...
    for fold_idx in range(args.k_fold):
        ...
        best_Dice, best_HD, best_ASD, best_IoU, best_ACC, best_SPE, best_SEN, best_clDice = train_net(net, args, ema_net, fold_idx=fold_idx)
        ...
        clDice_list.append(best_clDice)
    ...
    total_clDice = np.vstack(clDice_list)
    ...
    with open(f"{args.cp_dir}/cross_validation.txt",  'w') as f:
        ...
        f.write('clDice\n')
        for i in range(args.k_fold):
            f.write(f"Fold {i}: {clDice_list[i]}\n")
        f.write(f"Each Class clDice Avg: {np.mean(total_clDice, axis=0)}\n")
        f.write(f"Each Class clDice Std: {np.std(total_clDice, axis=0)}\n")
        f.write(f"All classes clDice Avg: {total_clDice.mean()}\n")
        f.write(f"All classes clDice Std: {np.mean(total_clDice, axis=1).std()}\n")
