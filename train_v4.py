...
class TopoLoss(nn.Module):
    def __init__(self):
        super().__init__()
        # 使用 BCEWithLogitsLoss，并加入 pos_weight
        # 血管骨架点占比极低，权重设为 10 到 20 之间比较合适
        self.bce = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([15.0]).cuda())
    ...
    def forward(self, skel_pred, gt_mask):
        ...
        gt_skel_small = F.interpolate(gt_skel, size=target_size, mode='nearest')
        
        # 3. 计算 Loss
        loss = self.bce(skel_pred, gt_skel_small)
        
        # 为了方便可视化，我们将生成的 gt_skel 挂载到对象属性上（可选）
        self.recent_skel_gt = gt_skel 
    ...
    for i, inputs in enumerate(trainLoader):
        ...
        def compute_total_loss(result, label, topo_weight=0.1):
            ...
            if isinstance(result, (list, tuple)):
                ...
                # --- [标准调用：触发 forward 方法] ---
                # 直接传入原始 label，让 TopoLoss 内部去处理生成和缩放
                loss_topo = criterion_topo(skel_pred, label)
                # ----------------------------------

                # 收集可视化数据 (从 criterion_topo 中拿回刚才生成的 GT)
        ...
        # --- [可视化逻辑：适配 Logits 与 Sigmoid 概率映射] ---
        if i % args.print_freq == 0:
            progress.display(i)
            
            # 只有当 vis_data 包含拓扑预测数据时才执行绘图
            if 'skel_pred' in vis_data:
                # 创建 1行5列 的画布
                fig, axs = plt.subplots(1, 5, figsize=(20, 4))
                
                # 1. 原始图像 (取 Batch 中的第一张)
                ...
                # 写入 TensorBoard 并在绘图后关闭 fig 以释放内存
