...
class TopologicalMediator(nn.Module):
    def __init__(self, in_channels, out_channels, topo_size=64, original_map_size=3):
        ...
        # [新增] 空间回传投影：将 1 通道骨架映射回 512 通道，且保持空间分辨率
    ...
    def forward(self, x):
        ...
        # 1. 上采样预测 (Loss 用)
        x_up = F.interpolate(x, size=(self.topo_size, self.topo_size), mode='bilinear', align_corners=True)
        skel_logits = self.skel_head(x_up) # [B, 1, 64, 64]
        topo_prob = torch.sigmoid(skel_logits)

        # 2. [新增] 空间回传流 (保留空间信息)
        # 将 64x64 下采样回 16x16，保留精确的坐标响应
        topo_spatial_small = F.interpolate(topo_prob, size=x.shape[-2:], mode='bilinear', align_corners=True)
        spatial_bias = self.spatial_proj(topo_spatial_small) # [B, 512, 16, 16]

        # 3. 全局回传流 (兼容 MedFormer 接口)
        topo_global_small = F.adaptive_avg_pool2d(topo_prob, (self.original_map_size, self.original_map_size))
        global_bias = self.global_proj(topo_global_small) # [B, 512, 3, 3]
        
        return skel_logits, spatial_bias, global_bias


class MedFormerV6(nn.Module):
