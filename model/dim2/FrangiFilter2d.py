...
def FrangiFilter2D(I, device):
    ...
    defaultoptions = {'FrangiScaleRange':(1,10), 'FrangiScaleRatio':1, 'FrangiBetaOne':1, 'FrangiBetaTwo':10,'BlackWhite':True};  
...
class FrangiFilter2d(nn.Module):
    def __init__(self):
        super(FrangiFilter2d, self).__init__()
    ...
    def forward(self, x):
        ...
        x = self.frangi_filter(input, input.device)
        # x = x.repeat(1, 3, 1, 1)
        return x