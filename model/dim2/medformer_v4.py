...
class TopologicalMediator(nn.Module):
    def __init__(self, in_channels, out_channels, map_size=64):
        super().__init__()
        # 建议 map_size 提高到 64 (24G 显存完全够用)
        self.map_size = map_size

        # 1. 骨架预测头：保持 3x3 卷积以增强连续性
        self.skel_head = nn.Sequential(
            ...
            # 【关键】移除这里的 torch.sigmoid，输出原始 Logits
        )
        ...
        # 3. 空间分辨率对齐
        self.pool = nn.AdaptiveAvgPool2d((self.map_size, self.map_size))

    def forward(self, x):
        # x: [B, C, H, W] (Encoder 特征)
        
        # 1. 在较高分辨率下预测骨架，保留细小血管的 Logits
        skel_logits = self.skel_head(x) 
        
        # 2. 调整到目标 map_size
        # 我们返回这个用于 TopoLoss 计算 (对应训练脚本里的 skel_pred)
        skel_out = self.pool(skel_logits) 
        
        # 3. 生成用于融合的拓扑偏置 (Bias)
        # 使用 sigmoid 后的激活值作为特征偏置，通常比 raw logits 更稳定
        topo_feat = torch.sigmoid(skel_out) 
        topo_bias = self.topo_proj(topo_feat) 
        
        return skel_out, topo_bias


class MedFormerV4(nn.Module):
