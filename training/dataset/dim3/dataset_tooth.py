...
class ToothDataset(Dataset):
    def __init__(self, args, mode="train", k_fold=5, k=0, seed=0):
        ...
        for name in selected_names:
            ...
            lab_name = f"{name}_gt.nii.gz"

            itk_img = sitk.ReadImage(os.path.join(args.data_root, img_name))
            itk_lab = sitk.ReadImage(os.path.join(args.data_root, lab_name))
    ...
    def __getitem__(self, idx):
        ...
        return tensor_img, tensor_lab, np.array(self.spacing_list[idx]), self.name_list[idx]
